...
End-to-end import pipeline running in production via the Replit-deployed migrate UI. As of **2026-06-08**:

- **Hybrid run loop is the current operating mode (Sept 2026).** Operator runs `docs/HYBRID-RUN.md` + `docs/OPERATOR-QUICKSTART.md` per merchant on a feature branch: recon → diagnose → import inactive → QA (`qa-store.ts` + visual pairs) → live fixes → mappings back into code → four-section write-back report in gitignored `migrations/<merchant>/`. Latest: Jack Henry (2026-09-10/11, 17 email + 4 hand-built OneText SMS flows, 12 fixes on `claude/jack-henry-mime-migration-84dcd3`); earlier: white-elm (09-04), bronco-western-supply (09-02). Parser now handles Klaviyo kl-table grids and kl-split event product cards; cart/browse grids use Redo's trigger-products sentinel.

- **Reviewer dashboard live in production.** Public-facing surface for external reviewers (non-Redo, non-Replit), deployed as a separate process on Michael's personal Replit plan at `https://mime-redo-general.replit.app/`. Same repo, same Postgres; the env var `MIME_SURFACE=public_review` strips admin/assist/import endpoints at boot via `dispatchReviewerSurface` (which runs before `checkBasicAuth`). Open access by design — `requireReviewer` falls through to a synthetic `PUBLIC_REVIEWER` row when no cookie is present (migration 012 seeds it). New tables / columns: `reviewers` (migration 010), `stores.created_by_reviewer FK` partial-indexed (migration 011). The dashboard is a hash-routed SPA (`#stores` / `#store=<id>` / `#job=<id>`) with a persistent right 1/4 jobs sidebar. Per-store flow / template / campaign picker, NDJSON live import stream, needs-input modal mirroring admin UX (boolean side-by-side buttons / choice click-to-submit list / text + Submit / Skip-this-item). Per-item notes (`POST /api/r/jobs/:id/notes`) write to the same `jobs.notes` column the admin's bundle download reads from. Cross-process notes sync via `refreshJobNotesFromDb` on `handleJobGet` + `handleJobBundle`. PRs #88 / #94 / #95 / #96 / #97 / #98 / #99 / #100 / #101 / #102 / #103 / #104 / #105 / #106. See `plans/2026-05-26-reviewer-dashboard.md` for the original spec.

- **Five production bundle fixes.** **#65** Browse Abandonment skip condition (Viewed Product / Active on Site both map to `MARKETING_BROWSE_ABANDONMENT`; layered `customer_activity[viewed-product]` skip cuts both ways). **#68** Conditional-split timeframe reads `tf.quantity` not `tf.value` — every migrated conditional split had been emitting "in last 1 day" for the entire corpus; SHOC 2026-05-21 surfaced it. **#107** CODE-template parser dropped `imageSourceType: "url"` (Redo enum is `{upload, product}` only now); also added `refreshJobNotesFromDb`. **#108** Block-editor parser extracts the color token from CSS `background` shorthand instead of passing the whole `#222222 url(...) center / repeat` string as `sectionColor` — that was the source of all the "Internal server error" 500s on `createSavedEmailTemplate` from SHOC's "Email with Large Graphic" template. **#109** Split 401 vs 403 in `import-rpc.ts` so permission errors (`{"error":"Lacking required permissions"}`) no longer loop on the JWT-refresh prompt; diagnosed live via Chrome MCP, fixed by adding the user as admin on Redo.

- **Bundle source survives Replit's stateless FS.** PRs **#69** + **#71**: `exported` / `flow_imported` event payloads now snapshot `klaviyoHtml` + `klaviyoMeta` / `klaviyoFlow`. Bundle endpoint falls back to event payload when `migrations/<slug>/{templates,flows}/` is empty (which it always is after a Replit restart). #71 then prunes the template snapshot after first bundle export (canonical-zip policy) so DB doesn't accumulate.
