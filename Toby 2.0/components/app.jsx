...
function App() {
  // ─ Stores ─
  const [stores, setStores] = useS(window.MOCK_STORES);
  const [view, setView] = useS({ screen: "dashboard", storeId: null });
  const [showAddStore, setShowAddStore] = useS(false);

  // ─ Per-store data catalogs (mock) ─
  // In reality each store would fetch its own templates/flows. We share the
  // same mock catalog across stores to keep this prototype simple.
  ...
  const retryItem = useC(async (jobId, item) => {
    ...
    const stream = window.mockRetryStream({ id: item.id, name: item.name, kind: item.kind, flow });
    for await (const evt of stream) applyEvent(jobId, evt);
    setJobs(js => js.map(j => j.id !== jobId ? j : ({
      ...j,
      items: j.items.map(i => i.id === item.id ? { ...i, retrying: false } : i),
    })));
  }, [jobs, data, applyEvent]);
      ...
      <div className="flex-1 overflow-hidden">
        {state.tab === "flows" ? (
          <ListShell
            ...
            emptyText="No flows match these filters."
